                    ...
                    switch (_a.label) {
                        case 0:
                            token = args[0].cookies
                                ...
                                : null;
                            if (token == null)
                                throw new error_middleware_1.CustomError("No token found", 403);
                            _token = store_global_1.global_login_store.verify_login_token(token);
                            args[0].user = _token;
                            args[0].token = token;
                            if (!_token) {
                                args[1].clearCookie("token");
                                return [2 /*return*/, args[1].json({ message: "Session expired" })];
                            }
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            ...
                            if (token == null)
                                throw new error_middleware_1.CustomError("No token found", 403);
                            _token = store_global_1.global_login_store.verify_staff_token(token);
                            args[0].user = _token;
                            args[0].token = token;
                            if (!_token) {
                                args[1].clearCookie("token");
